...
{
    fprintf(stderr, "usage: pipeline_codegen --dsl <file> [--output file] [--binary file] [--runs expr] [--define NAME=EXPR] [--compile] [--run]\n");
}

static bool parse_define_arg(const char *text, char *name, size_t name_size, char *value, size_t value_size)
{
...
int main(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; ++i)
    {
        ...
        {
            strncpy(options.binary_file, argv[++i], sizeof(options.binary_file) - 1);
        }
        else if (strcmp(argv[i], "--runs") == 0 && i + 1 < argc)
        {
            strncpy(options.runs_expr, argv[++i], sizeof(options.runs_expr) - 1);
        }
        else if (strcmp(argv[i], "--define") == 0 && i + 1 < argc)
        {
            if (options.define_count >= 32 ||
                !parse_define_arg(argv[++i],
                                  options.defines[options.define_count], sizeof(options.defines[options.define_count]),
                                  options.define_values[options.define_count], sizeof(options.define_values[options.define_count])))
            {
                usage();
                return 1;
            }
