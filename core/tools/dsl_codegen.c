...
static void emit_function_def(FILE *out, const FnDef *fn, const Chain *chains,
                              int chain_count) {
...
bool emit_program_c(const Program *program, const CodegenOptions *options) {
    ...
    for (int fi = 0; fi < program->function_count; ++fi) {
        emit_function_def(out, &program->functions[fi], program->chains,
                          program->chain_count);
    }
    ...
    for (int ci = 0; ci < program->chain_count; ++ci) {
        ...
        fprintf(out, "    List *result = init_list_with_capacity(data->size > "
                     "0 ? data->size : 1);\n");
        ...
        fprintf(out,
                "        if (value == (void *)(long)0XDEADBEEF) { break; }\n");
        for (int oi = 0; oi < chain->op_count; ++oi) {
    ...
    for (int ci = 0; ci < program->chain_count; ++ci) {
        fprintf(out, "        Observable *o_%d = range(%s, %s);\n", ci,
                program->chains[ci].range_min, program->chains[ci].range_max);
...
int compile_generated_program(const CodegenOptions *options) {
    ...
    snprintf(command, sizeof(command),
             "gcc -DENABLE_PROFILER=0 -Wall -Wextra -g -O0 -I./core/src %s "
