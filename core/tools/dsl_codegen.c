...
static void emit_expr(FILE *out, const Expr *expr, const char *value_name,
...
static void emit_function_def(FILE *out, const FunctionInfo *fn) {
    ...
    if (fn->kind == FUNCTION_PREDICATE) {
        ...
        fprintf(out,
                "static void *%s(void *accum, void *next)\n{\n    return (void "
                "*)(intptr_t)(",
                fn->def->name);
...
static void emit_fused_source_loop(FILE *out, const ChainIr *chain) {
    if (chain->source.kind == SOURCE_RANGE) {
        fprintf(out,
                "    for (intptr_t __src = (intptr_t)(%s); __src <= "
                "(intptr_t)(%s); ++__src)\n    {\n",
    ...
    if (chain->source.kind == SOURCE_OF) {
        fprintf(out, "    static const intptr_t __source_vals_%d[] = {",
                chain->op_count);
        ...
        fprintf(out, "};\n");
        fprintf(out,
                "    for (size_t __i = 0; __i < sizeof(__source_vals_%d) / "
                "sizeof(__source_vals_%d[0]); ++__i)\n    {\n",
                chain->op_count, chain->op_count);
        fprintf(
            out,
            "        void *value = (void *)(intptr_t)__source_vals_%d[__i];\n",
            chain->op_count);
        return;
    }
...
static void emit_fused_chain(FILE *out, const ChainIr *chain, int index) {
    ...
    bool uses_scan = chain_has_operator(chain, OP_SCAN) ||
                     chain_has_operator(chain, OP_SCANFROM);
    bool uses_last = chain_has_operator(chain, OP_LAST);
    bool uses_distinct_changed =
        chain_has_operator(chain, OP_DISTINCT_UNTIL_CHANGED);
    bool uses_take = chain_has_operator(chain, OP_TAKE) ||
                     chain_has_operator(chain, OP_FIRST);
    ...
    for (int oi = 0; oi < chain->op_count; ++oi) {
        ...
        case OP_TAKE:
            fprintf(out,
                    "        if (take_count >= (intptr_t)(%s)) { break; }\n",
                    op->extra);
        ...
        case OP_SKIP:
            fprintf(out,
                    "        if (skip_count < (intptr_t)(%s)) { skip_count++; "
                    "continue; }\n",
                    op->extra);
            break;
        case OP_TAKE_WHILE:
        ...
        case OP_SKIP_WHILE:
            fprintf(
                out,
                "        if (!skip_while_passed && %s(value)) { continue; }\n",
                op->symbol);
    ...
    if (uses_reduce) {
        ...
        fprintf(out,
                "    if (has_last_value) { push_back(result, last_value); }\n");
    }
...
static void emit_runtime_operator_expr(FILE *out, const OperatorAst *op,
    ...
    case ARGUMENT_LITERAL:
        if (op->kind == OP_MAP_TO || op->kind == OP_TAKE_UNTIL ||
            op->kind == OP_SKIP_UNTIL) {
            emit_value_as_ptr(out, op->extra);
        } else {
...
bool emit_program_c(const ExecutionPlan *plan, const CodegenOptions *options) {
    ...
    fprintf(out, "        int64_t ns = (int64_t)(end.tv_sec - start.tv_sec) * "
                 "1000000000LL + (int64_t)(end.tv_nsec - start.tv_nsec);\n");
