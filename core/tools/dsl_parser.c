...
static bool parse_operator(Parser *parser, Operator *op) {
    ...
    if (strcmp(name, "map") == 0) {
        op->kind = OP_MAP;
        if (!expect_ident(parser, op->function_name,
                          sizeof(op->function_name))) {
            return false;
        }
    ...
    if (strcmp(name, "reduce") == 0) {
        ...
        if (parser->lexer.current.kind != TOK_NUMBER &&
            parser->lexer.current.kind != TOK_IDENT) {
            fprintf(stderr, "parse error: expected reduce init literal\n");
            return false;
        }
        strncpy(op->literal, parser->lexer.current.text,
                sizeof(op->literal) - 1);
        lexer_next(&parser->lexer);
        return expect(parser, TOK_RPAREN, "')'");
    }
...
static bool parse_chain(Parser *parser, Chain *chain) {
    ...
    strncpy(chain->range_min, parser->lexer.current.text,
            sizeof(chain->range_min) - 1);
    lexer_next(&parser->lexer);
    ...
    strncpy(chain->range_max, parser->lexer.current.text,
            sizeof(chain->range_max) - 1);
    lexer_next(&parser->lexer);
