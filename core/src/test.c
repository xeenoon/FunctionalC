...
int main2() {
    ...
    // observable3 = observable3->pipe(observable3, map(take2), filter(IsEven),
    // NULL);
    ...
    // intervaltest = pipe(intervaltest, 2, buffer(intervaltest, intervaltest2),
    // distinctUntilChanged(lessThan3500));
