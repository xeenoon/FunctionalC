...
static bool stage_chain_is_streamable(CompiledStage *stages, int count) {
    ...
    for (int i = 0; i < count; ++i) {
        ...
        if (kind != QUERY_KIND_MAP && kind != QUERY_KIND_MAP_CHAIN &&
...
static void ensure_compiled_pipeline(Observable *o) {
    ...
    compiled->stages =
        count > 0 ? malloc((size_t)count * sizeof(CompiledStage)) : NULL;
    compiled->count = 0;
    ...
    while (current != NULL && current->emit_handler != NULL) {
        ...
        if (query->kind == QUERY_KIND_MAP) {
            ...
            while (scan != NULL && scan->emit_handler != NULL &&
            ...
            if (map_count == 1) {
                ...
                chain->funcs =
                    malloc((size_t)map_count * sizeof(ModifierFunction));
    ...
    compiled->streamable =
        stage_chain_is_streamable(compiled->stages, compiled->count);
    if (compiled->count > 1 && compiled->stages[0].query != NULL &&
        compiled->stages[0].query->func == mergemap_apply) {
        compiled->stream_start_index = 1;
        compiled->mergemap_then_streamable =
            stage_chain_is_streamable(&compiled->stages[1],
                                      compiled->count - 1);
    }
...
static void pop_all_streaming_compiled(Observable *root, CompiledStage *stages,
                                       int stage_count, List *data,
        ...
        for (int stage_index = 0; stage_index < stage_count; ++stage_index) {
            ...
            if (stage->kind == QUERY_KIND_MAP) {
                ...
                DistinctUntilChangedCtx *ctx =
                    (DistinctUntilChangedCtx *)stage->ctx;
...
void pop_all(Observable *o) {
    ...
    if (o->emit_handler && query_chain_is_streamable(o)) {
        pop_all_streaming_compiled(o, o->compiled_pipeline->stages,
                                   o->compiled_pipeline->count, data,
                                   total_start, PROFILE_NOW_NS());
        return;
    }

    ensure_compiled_pipeline(o);
    if (o->emit_handler && o->compiled_pipeline != NULL &&
