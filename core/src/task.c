...
// Insert in sorted order by time_ms (ascending). Returns true if inserted at
// head.
...
static void *tasks_worker(void *unused) {
    ...
    while (tasks_running) {
        ...
        if (tasks_head->time_ms <= now) {
            // Pop and run all tasks that are due now (in FIFO order for equal
            // times)
            ...
            // Wait until the head is due OR until signalled
            // (insert/cancel/shutdown)
            ...
            int rc =
                pthread_cond_timedwait(&tasks_cond, &tasks_mutex, &timeout);
            if (rc == ETIMEDOUT) {
                // will loop and execute the due task
                continue;
            } else {
                // woken by signal (insertion/shutdown). Loop will recompute
                // head/now.
                continue;
            }
        }
